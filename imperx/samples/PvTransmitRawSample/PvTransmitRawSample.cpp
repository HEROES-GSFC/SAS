...
// PvTransmitterRaw classes to transmit raw data (not images) using the eBUS 
// SDK.
//
...
//
// Using the PvTransmitterRaw class, a stream of raw buffers is transmitted to 
// the specified destination.
//
// By default, this sample transmits raw buffers with continuously varying data 
// from the first valid interface to multicast address 239.192.1.1:1042. 
// However, it can be used in many different ways by providing it with optional 
// command line arguments. For further details on the command line options that 
// are available, run the sample with the argument --help.
// *****************************************************************************
...
int main( int aCount, const char ** aArgs )
{
    ...
    {
        // Alloc new buffer
        PvBuffer *lBuffer = new PvBuffer( PvPayloadTypeRawData );
        lBuffer->GetRawData()->Alloc( lConfig.GetDataSize() );

        // Set to 0
        memset( lBuffer->GetDataPointer(), 0x00, lConfig.GetDataSize() );
    ...
    if ( lConfig.GetFPS() != 0 )
    {
        ...
        float lMax = static_cast<float>( lConfig.GetDataSize() ) * 8;
        lMax *= lConfig.GetFPS();
    ...
    while( !PvKbHit() )
    {
        ...
        if ( ( lConfig.GetFPS() == 0 ) || lStabilizer.IsTimeToDisplay( (PvUInt32)lConfig.GetFPS() ) )
        {
            ...
            if ( lFreeBuffers.size() > 0 )
            {
                ...
                {
                    unsigned char lValue = lSeed;
                    unsigned char *lPtr = lBuffer->GetDataPointer();

                    PvUInt32 lSize = lConfig.GetDataSize();
                ...
                lResult = lTransmitter.QueueBuffer( lBuffer );
                if( lResult == PvResult::Code::NO_LICENSE )
                {
                    printf("This sample requires a transmitter license to work");
                    break;
                }
            }
        }
